"""Account setup script — creates and funds 3 test employees, opts them in, registers them.

Usage:
    python scripts/fund_accounts.py --network localnet --app-id <ID> --asset-id <ID>

Steps per employee:
    1. Generate random account
    2. Fund with ALGO (for MBR + fees)
    3. Opt into PAYUSD ASA
    4. Opt into PayrollStream app
    5. Register with employer at specified hourly rate
"""

# Standard library
import argparse
import os
import sys
import uuid
from pathlib import Path

# Third-party
from algosdk.transaction import (
    AssetTransferTxn,
    wait_for_confirmation,
)
from algosdk.atomic_transaction_composer import TransactionWithSigner

# AlgoKit utilities
from algokit_utils import (
    AlgorandClient,
    AlgoAmount,
    AppClient,
    AppClientParams,
    AppClientMethodCallParams,
    AppClientBareCallParams,
    AssetOptInParams,
    PaymentParams,
    Arc56Contract,
)

# Project root
PROJECT_ROOT = Path(__file__).parent.parent

# ARC56 spec path
APP_SPEC_PATH = PROJECT_ROOT / "smart_contracts" / "payroll_stream" / "PayrollStream.arc56.json"

# Employee configuration: (label, hourly_rate_in_display_units)
EMPLOYEE_CONFIGS = [
    ("Alice", 100_000_000),   # 100.000000 PAYUSD/hr
    ("Bob", 75_000_000),      # 75.000000 PAYUSD/hr
    ("Charlie", 50_000_000),  # 50.000000 PAYUSD/hr
]

# ALGO funding per employee (for MBR + opt-ins + fees)
EMPLOYEE_ALGO_FUND = 5  # 5 ALGO

# Token decimals for display
PAYUSD_DECIMALS = 6

# Confirmation rounds
TX_CONFIRMATION_ROUNDS = 4


def parse_args() -> argparse.Namespace:
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Fund and register test employee accounts for AlgoFlow"
    )
    parser.add_argument(
        "--network",
        choices=["localnet", "testnet"],
        default="localnet",
        help="Target network (default: localnet)",
    )
    parser.add_argument(
        "--app-id",
        type=int,
        required=True,
        help="Deployed PayrollStream application ID",
    )
    parser.add_argument(
        "--asset-id",
        type=int,
        required=True,
        help="PAYUSD ASA ID",
    )
    return parser.parse_args()


def get_algorand_client(network: str) -> AlgorandClient:
    """Create an AlgorandClient for the given network."""
    if network == "localnet":
        return AlgorandClient.default_localnet()
    else:
        return AlgorandClient.testnet()


def get_employer(algorand: AlgorandClient, network: str):
    """Get the employer/deployer account.

    Args:
        algorand: The AlgorandClient.
        network: 'localnet' or 'testnet'.

    Returns:
        SigningAccount for the employer.
    """
    if network == "localnet":
        return algorand.account.localnet_dispenser()
    else:
        deployer_mnemonic = os.environ.get("DEPLOYER_MNEMONIC", "")
        if not deployer_mnemonic:
            print("ERROR: DEPLOYER_MNEMONIC environment variable is required for testnet.", file=sys.stderr)
            sys.exit(1)
        return algorand.account.from_mnemonic(mnemonic=deployer_mnemonic)


def get_app_client(algorand: AlgorandClient, app_id: int, employer_address: str) -> AppClient:
    """Load the AppClient for an existing deployed contract.

    Args:
        algorand: The AlgorandClient.
        app_id: The deployed application ID.
        employer_address: The employer's address (default sender).

    Returns:
        AppClient instance.
    """
    if not APP_SPEC_PATH.exists():
        print(f"ERROR: ARC56 spec not found at {APP_SPEC_PATH}", file=sys.stderr)
        sys.exit(1)

    with open(APP_SPEC_PATH) as f:
        app_spec = Arc56Contract.from_json(f.read())

    return AppClient(
        AppClientParams(
            algorand=algorand,
            app_spec=app_spec,
            app_id=app_id,
            default_sender=employer_address,
        )
    )


def create_and_fund_employee(
    algorand: AlgorandClient,
    employer,
    label: str,
    index: int,
) -> object:
    """Create a random employee account and fund it with ALGO.

    Args:
        algorand: The AlgorandClient.
        employer: The employer signing account (funds source).
        label: Human-readable label for the employee.
        index: Employee index (1-based).

    Returns:
        The employee SigningAccount.
    """
    employee = algorand.account.random()
    print(f"    [{index}] {label}: {employee.address}")

    algorand.account.ensure_funded(
        employee.address,
        employer.address,
        AlgoAmount.from_algo(EMPLOYEE_ALGO_FUND),
    )
    print(f"        Funded with {EMPLOYEE_ALGO_FUND} ALGO")

    return employee


def opt_employee_into_asa(algorand: AlgorandClient, employee, asset_id: int, label: str) -> None:
    """Opt an employee into the PAYUSD ASA.

    Args:
        algorand: The AlgorandClient.
        employee: The employee signing account.
        asset_id: The PAYUSD ASA ID.
        label: Human-readable label for the employee.
    """
    algorand.send.asset_opt_in(
        AssetOptInParams(
            sender=employee.address,
            asset_id=asset_id,
        )
    )
    print(f"        {label} opted into PAYUSD ASA (ID: {asset_id})")


def opt_employee_into_app(
    app_client: AppClient,
    employee,
    label: str,
) -> AppClient:
    """Opt an employee into the PayrollStream app.

    Args:
        app_client: The employer's AppClient.
        employee: The employee signing account.
        label: Human-readable label for the employee.

    Returns:
        The employee's AppClient (cloned with employee as sender).
    """
    employee_client = app_client.clone(default_sender=employee.address)
    employee_client.send.bare.opt_in(
        AppClientBareCallParams(
            note=f"algoflow:emp-optin-{label}-{uuid.uuid4()}".encode(),
        )
    )
    print(f"        {label} opted into PayrollStream app (ID: {app_client.app_id})")
    return employee_client


def register_employee(
    app_client: AppClient,
    employee_address: str,
    rate: int,
    label: str,
) -> None:
    """Register an employee with the PayrollStream contract.

    Args:
        app_client: The employer's AppClient.
        employee_address: The employee's Algorand address.
        rate: Hourly salary rate in base units.
        label: Human-readable label for the employee.
    """
    app_client.send.call(
        AppClientMethodCallParams(
            method="register_employee",
            args=[employee_address, rate],
            note=f"algoflow:register-{label}-{uuid.uuid4()}".encode(),
        )
    )
    display_rate = rate / 10**PAYUSD_DECIMALS
    print(f"        {label} registered at {display_rate:,.6f} PAYUSD/hr")


def main() -> None:
    """Run the account setup pipeline."""
    args = parse_args()
    network = args.network
    app_id = args.app_id
    asset_id = args.asset_id

    print(f"AlgoFlow Account Setup — target: {network}")
    print(f"  App ID:   {app_id}")
    print(f"  Asset ID: {asset_id}")
    print("-" * 50)

    # Load .env
    env_path = PROJECT_ROOT / ".env"
    if env_path.exists():
        try:
            from dotenv import load_dotenv
            load_dotenv(env_path)
        except ImportError:
            for line in env_path.read_text().splitlines():
                line = line.strip()
                if line and not line.startswith("#") and "=" in line:
                    key, _, value = line.partition("=")
                    os.environ.setdefault(key.strip(), value.strip())

    # Connect
    algorand = get_algorand_client(network)
    print(f"Connected to {network}")

    # Get employer
    employer = get_employer(algorand, network)
    print(f"Employer: {employer.address}")

    # Get AppClient
    app_client = get_app_client(algorand, app_id, employer.address)

    # Process each employee
    employees = []
    print("\nCreating and registering employees:")
    print("=" * 50)

    for i, (label, rate) in enumerate(EMPLOYEE_CONFIGS, start=1):
        print(f"\n  Employee {i}: {label}")
        print(f"  {'─' * 40}")

        # Create and fund
        employee = create_and_fund_employee(algorand, employer, label, i)

        # Opt into ASA
        opt_employee_into_asa(algorand, employee, asset_id, label)

        # Opt into app
        employee_client = opt_employee_into_app(app_client, employee, label)

        # Register with contract
        register_employee(app_client, employee.address, rate, label)

        employees.append({
            "label": label,
            "address": employee.address,
            "rate": rate,
            "client": employee_client,
        })

    # Summary
    print("\n" + "=" * 60)
    print("  Employee Setup Summary")
    print("=" * 60)
    for emp in employees:
        display_rate = emp["rate"] / 10**PAYUSD_DECIMALS
        print(f"  {emp['label']:10s} | {emp['address']} | {display_rate:>12,.6f} PAYUSD/hr")
    print("=" * 60)
    print(f"\n  Total employees registered: {len(employees)}")
    print(f"  App ID: {app_id}")
    print(f"  Asset ID: {asset_id}")
    print("\nDone!")


if __name__ == "__main__":
    main()
